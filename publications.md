...
<style>
  ...
  .pub-categories button:hover {
    background: rgba(15, 23, 42, 0.06);
    color: #0f172a;
  }

  .pub-categories button.active {
    background: #0f766e;
    color: #fff;
...
<div class="modal-overlay" id="pub-modal" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-abstract">
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function openFirstYear(section) {
      ...
      const firstToggle = firstList.previousElementSibling;

      firstList.classList.add('active');
      if (firstToggle && firstToggle.classList.contains('year-toggle')) {
    ...
    categoryButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        const current = document.querySelector('.pub-categories button.active');
        if (current) {
    ...
    document.querySelectorAll('.abstract-btn').forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        modalToc.alt = btn.dataset.toc ? 'TOC Figure' : '';
    ...
    const initial = document.querySelector('.pub-categories button.active');
    if (initial) {
      setActiveCategory(initial.dataset.cat);
    }
  });
</script>
